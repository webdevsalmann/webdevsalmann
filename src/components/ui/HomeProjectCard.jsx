...
export default function HomeProjectCard({ position, title, concept, desc, link, img }) {
    return (
        <motion.div
            initial={{ x: position % 2 === 0 ? -30 : 30, visibility: "hidden" }}
            whileInView={{ x: 0, visibility: "visible" }}
            transition={{ duration: 0.3 }}
            viewport={{ once: true }}
            className={`relative w-full md:w-4/5 rounded-md overflow-hidden transition-all group ${position % 2 === 0 ? "self-start" : "self-end"}`}
        >
